...
AgentOS 采用"具身认知"的工程实现——从身体（工具层）到自我认知（自主层），逐层向上构建：

```
┌──────────────────────────────────────────────────────────────┐
│                 AI 通用行动代理操作系统 (AgentOS)              │
│                                                               │
...
复杂任务可能涉及多个工具协作。AgentOS 追踪每个步骤的完成情况，定位瓶颈（"周报流程中 Excel 是瓶颈"）。
...
不同工具有不同的成功/失败信号。AgentOS 为每个工具定义反馈→学习的映射：
