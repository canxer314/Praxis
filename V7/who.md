...
开发者不参与 AgentOS 的日常运行。开发者构建和迭代 AgentOS 插件本身。
...
- ❌ 不需要替用户设计认知结构（那是 AgentOS + 用户协作的事）
- ❌ 不需要理解每个用户的业务领域（那是 AgentOS 学的事）
- ❌ 不需要手动管理 AgentMemory 中的数据（那是 AgentOS 编排的事）
...
运维者负责 AgentOS 的运行环境健康和高级架构治理。运维者不一定是用 AgentOS 完成日常任务的人。

| 职责 | 具体工作 |
|------|---------|
| **架构版本管理** | 审批 Level 3+ 的结构变更，管理架构版本号 |
| **监控与告警** | 关注退化报告、性能指标、异常冻结 |
| **紧急干预** | 执行 `/agentos architecture freeze`，回滚架构版本 |
...
| 场景 | 用户 | 运维者 |
|------|------|--------|
| AgentOS 提议一个新认知结构 | 审核业务合理性（"这个结构对吗？"） | 审核架构一致性（"这个结构放在哪里？"） |
| 核心指标退化 > 5% | 收到通知（"AgentOS 表现变差了"） | 收到告警 + 启动诊断（"哪里出问题了？"） |
...
```
AgentOS 进入陌生场景 → 只观察，不提问
  ├─ 在系统提示中静默注入感知指令
  ├─ session_end 分析 SalientElement
  ├─ AgentOS 不主动打断用户
  └─ 用户可以用 /agentos perceive 查看 AgentOS 观察到了什么
```
...
```
ProtoStructure 置信度达标 → AgentOS 提议固化:

  "/agentos crystallize 提议:
   将 '医院门诊流程' 固化为可复用模板
   观察次数: 8, 核心置信度: 0.85
   固化为: ProcessTemplate 'hospital_outpatient_visit'
   
   批准? /agentos crystallize approve
   拒绝? /agentos crystallize reject [原因]
   修改? /agentos crystallize edit '描述修正'"
```
...
| 命令 | 功能 | Phase |
|------|------|-------|
| `/agentos perceive` | 查看当前场景的感知状态 | Phase 1 |
| `/agentos proto <id>` | 查看原型详情 + 置信度历史 | Phase 2 |
| `/agentos proto <id> correct <描述>` | 纠正某个原型 | Phase 2 |
| `/agentos crystallize` | 查看待审批的固化提案 | Phase 3 |
| `/agentos crystallize approve` | 批准固化 | Phase 3 |
| `/agentos crystallize reject <原因>` | 拒绝固化 | Phase 3 |
| `/agentos architecture status` | 查看当前架构版本 | Phase 4 |
| `/agentos architecture freeze` | 紧急冻结 | Phase 4 |

---

## 五、AgentOS 自身的"自主权边界"

V7 从工程角度定义 AgentOS 可以"自主"做什么（不需要人类审批）：
