...
2. 用户手动管理任务:
   /agentos task start "构建医院管理系统"
     → 创建新 TaskContext
   
   /agentos task update --phase "Phase 3: UI Development"
     → 更新任务阶段
   
   /agentos task progress "API 全部完成, 开始前端"
     → 更新进度摘要
...
决策 1: TaskContext 由谁更新？
  ...
  方案 C: LLM 推断 + 用户可覆盖 → 推荐
    ...
    用户可随时用 /agentos task update 覆盖自动推断
...
## 五、AgentOS 自身的"自主权边界"（与 V9 相同）

V10 不改变 AgentOS 的自主权边界。TaskContext 是一个索引——它不驱动行动，只提供上下文。
