...
**仍待修复 (后续)**:
- ✅ T1: M4 三个独立验证器接入运行时融合 (Phase 2, 2026-06-27)
- ✅ T6: M1 关系图置信度传播接入运行时 (Phase 2, 2026-06-27)
- ✅ B6: session_start 加载完整结构 (Phase 0, 2026-06-27)
- ✅ T9: bridge→EventOrchestrator 迁移 + daemon 调度 (Phase 1, 2026-06-28) — HookDispatcher per-hook entry + /praxis CLI 扩展 + praxis-daemon.ts (D2=daemon, setInterval 30min)
