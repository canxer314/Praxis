...
### 目标
消除 V8 在复杂连续任务中 token 爆炸的系统性风险。Phase 0 完成后，Praxis 在任何上下文压力下都能优雅运行。
...
| **遥测仪表盘 (基础)** | `/praxis telemetry` — 结构采用率排行 | P1 |
...
✅ 压力测试: 极端场景下 Praxis 成功降级到 Critical 模式 → 不崩溃
