...
> 以下修订来自 `docs/` 中本体论分析（哲学+工程）和 ROADMAP 第一性原理推导的合成。  
> 修订不是增加新系统，而是对已有设计概念增加精确性、显式命名关键设计决策、填补表述缺口。

### §1 — 增加"概率化计算本体论引擎"身份陈述

**之前**: 只以"大脑皮层"和 Context Orchestration Layer 定义 Praxis。  
**修订**: 增加精确工程定义——"概率化的、自我演化的计算本体论引擎"，并点明与传统知识图谱的关键区别。增加"核心张力: Feature List vs World Model"的显式命名。

### §3 — 三种本体论立场（ontologyStance）可配置

**之前**: bootstrap 和纯观察驱动被描述为两条独立学习路径，但没有被框架化为可配置策略。  
**修订**: 增加"三种本体论立场"小节——实在论（bootstrap 预设）、唯名论（纯观察）、概念论（LLM 归纳）。三种立场可混合配置 weight。新增 `ontology_stance` 到 GovernancePolicy 配置项。

### §3 — 结晶化条件展开为奎因三重判准

**之前**: 结晶化为四重门控，其中"反事实检验"是一个笼统的条件。  
**修订**: 展开为五重门控——必要性检验（不假设该结构存在能否解释行为？）、充分性检验（假设后预测准确率是否更高？）、奥卡姆剃刀（有无更简单的替代？）。三者合称"奎因式反事实检验"，对应奎因的"存在就是成为约束变元的值"判准。

### §7 — 粒度从单维升级为双维（token 预算 × 认知成熟度）

**之前**: 粒度控制只考虑四级压力（按 token 数压缩）。  
**修订**: 增加"认知成熟度驱动的语义粒度"小节——同一 token 预算下，Novice/Competent/Expert 的注入信息密度不同。两个维度交互决定注入策略。

### §8 — 范畴审计增加康德式区分 + 领域范畴同质性检查

**之前**: 范畴审计只问"类型是否够用"。  
**修订**: 
- 增加康德式诊断分叉: "数据问题还是范畴问题？"——避免将"数据不够"误判为"范畴不够"。
- 增加领域范畴同质性检查: 不同工程领域（软件/物理/业务）可能需要不同的 ProtoStructure 子类型，不应强行统一。

### §12 — 增加第 9 条设计原则: "神经符号架构"

**之前**: 8 条设计原则分别描述了各项设计选择，但没有命名 Praxis 的终极架构形态。  
**修订**: 增加第 9 条——"神经符号架构"。符号侧（ProtoStructure + 约束公理 + 统计验证器）保证可靠性和可审计性，神经侧（LLM）保证灵活性和泛化。两侧的边界和权重是 Praxis 最核心的架构决策。

### 新增 §13 — 用户可见的认知状态

**之前**: 没有描述用户如何查阅 Praxis 的内部认知。  
**修订**: 新增章节——`/praxis ontology`（本体论承诺完整清单）、`/praxis status`（能力模型+成长轨迹）、`/praxis audit`（Meta Layer 审计报告）、`/praxis task`（任务编排状态）。实践奎因的原则——本体论承诺不应是私人的形而上学信念，而应可公开审计。

### 修订背后的设计方法修正

**之前的第一性原理分析结论**: I-A（关系图）是"一切后续能力的前置条件"，应最高优先级。  
**反向论证后修正**: II-A（约束注入）和 III-A（注意力遥测）不依赖 I-A。正确的策略是**双轨并行**:
- 轨道 1: II-A + III-A — 快速交付用户价值 + 积累生产数据
- 轨道 2: I-A — 深层基础设施建设
