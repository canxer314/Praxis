# How does AgentOS V2 work?

## 总览：AgentOS Plugin 内部架构

AgentOS V2 仍然保持六层架构，但运行在 OpenClaw 的 Memory Plugin 槽位内。每一层通过 OpenClaw Plugin Hooks 与外部交互：
...
│  └──────────────────────────┬───────────────────────────────┘  │
│                             │                                   │
│  OpenClaw 原生工具执行      │  AgentOS 不直接执行工具           │
...
在 V2 中，工具不由 AgentOS 注册或执行。工具属于 OpenClaw（native tools）或 MCP Servers（external tools）。AgentOS 的工具层只做一件事：**管理工具的元数据和熟练度**。

### 工具元数据（可选）

工具开发者可以选择为 OpenClaw Plugin 或 MCP Server 提供 `agentos` 元数据：
...
如果没有提供元数据，AgentOS 从零开始学习——慢，但不阻塞。
...
AgentOS 不需要手动注册工具。它在 `session_start` 时通过 OpenClaw 的已加载工具列表发现可用的工具，然后从 AgentMemory 的 `tool_registry` slot 中加载已有的熟练度数据。新工具自动以 `novice` 级别开始。

---

## Layer 2: 任务编排层 —— AI 的"行动"

V2 中任务编排的"执行"部分由 OpenClaw agent loop 负责。AgentOS 的任务层负责"追踪"和"评估"。
...
用户分配任务（通过 OpenClaw 通道）
  ...
  → AgentOS 在 before_tool_call 注入上下文
  → OpenClaw 执行工具
  → AgentOS 在 after_tool_call 追踪步骤
  → [循环直到任务完成]
  → OpenClaw agent_end
  → AgentOS 在 agent_end 评估整条任务链
  → 提取学习事件，更新能力模型
```
...
| V1 | V2 |
|----|----|
| AgentOS 负责任务分解和执行 | OpenClaw 负责分解和执行，AgentOS 追踪 |
...
在 `before_tool_call` 中，AgentOS 自动检索与该工具/任务相关的知识：

```
before_tool_call(tool="ppt_generator", action="create_presentation")
  → AgentOS: memory_smart_search("PPT设计原则 user:canxe", type="knowledge")
  → 找到: "图表优先，文字精简，深色模板"
...
```
┌──────────────────────────────────────────────────────────┐
│              Learning Loop (in AgentOS Plugin)            │
│                                                           │
...
反馈解释器的输入从 AgentOS 自己的工具调用结果变为 OpenClaw 的工具调用结果。但由于两者最终都是调用同一个 MCP Server，返回格式一致，因此反馈解释器逻辑**不变**。
...
当 OpenClaw 准备执行一个工具操作时 before_tool_call：
  1. AgentOS 查该 tool_id 的 proficiency
  2. 查该操作类型的风险等级
  3. 决定：
     - 熟练 + 低风险 → "proceed"（自主执行）
...
```typescript
// AgentOS Plugin 的 before_tool_call handler 内部逻辑
