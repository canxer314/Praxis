...
2. **OpenClaw 运维者** — 部署 OpenClaw Gateway + Praxis Plugin 的人
...
通过 OpenClaw 的任意通道与 AI 交互。他们不直接接触 Praxis——Praxis 在背后让 AI 越来越好用。
...
- 审核成长："让我看看你现在 PPT 能力到哪个水平了" → Praxis 通过 `/praxis status` 返回能力报告
...
**协作者做的事**：
- 直接分配复杂任务，不需要详细指示
- 只在关键决策点参与确认（Praxis 的 autonomy engine 自动判断哪些操作需要确认）
- 把 AI 当作"团队里的一个熟手"
...
这是 V2 的新角色。部署和维护 OpenClaw Gateway + Praxis Plugin 的人。

**运维者做的事**：
- 部署 OpenClaw Gateway（`openclaw onboard`）
- 安装 Praxis Plugin（`openclaw plugins install praxis`）
- 配置 AgentMemory 连接（MCP endpoint + credentials）
- 在 `openclaw.json` 中设置 `plugins.slots.memory = "praxis"`
...
这是变化最大的角色。V1 中工具开发者直接面向 Praxis 的工具注册 API。V2 中工具开发者面向的是 OpenClaw / MCP 生态。
...
- 工具不需要任何修改就能接入 Praxis 的学习闭环（Praxis 需要反馈解释器，但可以从默认模板开始）
- 如果提供了 `praxis` 元数据 → 学习速度更快，自主性判断更准确
- 如果没有提供 → Praxis 从零开始学习（慢但不阻塞）

**关键变化**：工具开发者的目标平台从 "Praxis Tool Registry" 变为 "MCP Server" 或 "OpenClaw Plugin"。Praxis 不再拥有工具的所有权——它只是观察者和加速器。

---

## 四、AI 的"自我"：Praxis 在培养谁？

**不变的核心命题**：

> **Praxis 培养的不是一个预设的"AI 人格"，而是一个从行动中涌现的"专业身份"。**
...
因为 Praxis 在 OpenClaw 内部运行，AI 的"身体边界"由 OpenClaw 的工具生态定义：
- AI 可以通过 OpenClaw 的 20+ 通道与用户交互
- AI 可以操作 OpenClaw 支持的所有 MCP 工具
- AI 的"我能做什么" = OpenClaw 已安装的工具集 + Praxis 追踪的熟练度
...
```
OpenClaw Gateway
  ├─ Agent "work"   → Praxis scope="work"   → AgentMemory project scope
  ├─ Agent "home"   → Praxis scope="home"   → AgentMemory project scope
  └─ Agent "coding" → Praxis scope="coding" → AgentMemory project scope
```
...
| **Praxis 在培养谁？** | 从行动中涌现的专业身份——通过 OpenClaw 的工具执行被证明 |
| **工具开发者的变化？** | 面向 MCP/OpenClaw Plugin SDK，Praxis 是可选的加速元数据 |
