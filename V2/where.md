# Where does AgentOS V2 sit?

## 架构定位（V2）

AgentOS V2 位于 **OpenClaw 的内部**（作为 memory plugin）和 **AgentMemory 的前端**（作为 MCP client）。它不是一个独立运行的进程——它是 OpenClaw runtime 的一部分，在 plugin slot 中运行。
...
| 维度 | V1 | V2 |
|------|----|----|
| AgentOS 位置 | LLM 和世界之间（独立 Harness）| OpenClaw 内部（Memory Plugin） |
| 工具执行 | AgentOS 直接编排 | OpenClaw 原生执行，AgentOS 观察 |
...
| 工具注册 | AgentOS Tool Registry | OpenClaw 工具生态 + AgentOS 元数据层 |

## AgentOS 与各系统的关系

### AgentMemory（存储后端）

**关系类型**：组合（AgentOS Plugin 通过 MCP 调用 AgentMemory）

与 V1 的关系不变。AgentOS 通过 OpenClaw 的 MCP 客户端能力连接到 AgentMemory MCP Server。所有存储操作（slot 读写、lesson 存储、知识检索）通过 MCP 协议。

### OpenClaw（运行宿主）

**关系类型**：宿主-插件（AgentOS 是 OpenClaw 的 memory plugin）

这是 V2 最大的架构变化。AgentOS 不控制 OpenClaw——OpenClaw 控制 agent loop，AgentOS 在钩子点响应事件。这类似于一个浏览器扩展：扩展不能改变浏览器的渲染引擎，但可以在页面加载、用户点击等事件中插入自己的逻辑。

### MCP Servers（工具提供者）

**关系类型**：间接（OpenClaw 连接 MCP → AgentOS 观察工具使用）

在 V1 中 AgentOS 直接管理和调用 MCP 工具。在 V2 中 AgentOS 不直接调用工具——它观察 OpenClaw 的工具调用，追踪熟练度，在适当的时候影响自主性决策。

### Hermer / PI（互补系统）

**关系类型**：互补，非集成

- **PI**：作为 OpenClaw 的一个 coding tool（不是 AgentOS 的组成部分）
- **Hermes**：作为 AgentOS 的**替代方案参考**（Hermes 内置学习闭环 vs AgentOS 独立学习闭环）
...
- **single**：一个 OpenClaw agent + 一个 AgentOS plugin + 一个 AgentMemory 实例
