...
> **一句话**：Praxis V2 是运行在 OpenClaw 内部的一个 memory plugin，它把 AI 的工具使用经验转化为可量化、可累积、可跨会话传递的能力模型。

## V2 的本质变化

V1 中 Praxis 被设想为一个独立的 Harness 层中间件，需要自建完整的 agent runtime（Hook 系统、工具执行、通道管理）。

V2 重新审视后确认：**Praxis 的核心价值是学习闭环 + 能力模型 + 自主决策**。工具执行、通道通信、agent loop 是"身体"的职责——这恰好是 OpenClaw 已经做好的事。
...
| 工具注册在 Praxis 内部 | 工具注册 = OpenClaw 的工具生态 + Praxis 附加 proficiency/feedback 元数据 |
| `/praxis` 命令是 Claude Code Skills | `/praxis` 命令是 OpenClaw Plugin 暴露的 slash commands |
...
```
第 1 次：你一步步在 Telegram 里教它怎么冲。Praxis 记录你的参数偏好到 AgentMemory。

第 3 次：它在 Telegram 上说"我帮你冲咖啡"。OpenClaw 调用 coffee_machine MCP 工具。
         Praxis 的 before_tool_call hook 检查到 coffee_machine proficiency=0.35，
         决定需要确认："准备用 85°C 冲深烘焙，200ml，可以吗？"
         它忘记检查水量——失败了。after_tool_call 检测到 out_of_water 信号，
         Praxis 生成 LearningEvent: "冲之前必须检查 status()"。
...
第 50 次：它注意到你周一早上和周五下午的口味偏好不同，
         ...
         Praxis 在 memory plugin slot 中默默记录每一次的 improvement。
```

**V2 的关键点**：用户通过 OpenClaw 的原生通道（Telegram/Discord/WhatsApp/...）与 AI 交互，OpenClaw 执行工具，Praxis 在背后追踪一切并让下一次更好。用户不需要知道 Praxis 的存在——就像你不需要知道大脑的杏仁核在做什么。
