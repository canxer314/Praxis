...
> 从第一性原理出发，分析为什么 V11 的四个结构化接口是"在错误边界上建造的桥梁"，为什么 AgentOS 应该直接接管 planning-with-files 的认知职能。
...
事实 2: AgentOS 的 ProtoTask 是天然的任务分解引擎。
...
事实 3: V11 的四个结构化接口中，有三个是为了跨越一个本不必要存在的边界。
        KnowledgeQuery 让 planning-with-files "查询" AgentOS 的 ProtoTask。
        GuidanceSignal 让 AgentOS 向 OpenClaw "发送" 认知指导。
        OutcomeFeedback 让 OpenClaw 向 AgentOS "反馈" 任务结果。
        如果 AgentOS 自己做任务分解，这三个"接口"都是内部函数调用。

事实 4: planning-with-files 的任务分解质量完全依赖 LLM 的通用知识。
        做了 10 次同类项目后，planning-with-files 仍然从零开始生成计划——
        因为它是无状态的 prompt 模板，不学习。

事实 5: OpenClaw 的 plugin API 完全支持 AgentOS 作为"主动"引擎运行。
...
```
AgentOS (知)  ←──四个结构化接口──→  planning-with-files / OpenClaw (行)
```
...
如果把 planning-with-files 的 SKILL.md 内容直接嵌入 AgentOS 的 prompt 模板中——功能上没有任何区别。但架构图会变得"不对称"，因为少了一个"层"。
...
接口 1 (KnowledgeQuery):
  planning-with-files → 查询 → AgentOS → 返回 ProtoTask
  问题: 为什么 planning-with-files 需要"查询" AgentOS？
        因为 planning-with-files 负责"任务分解"，但它没有知识。
        所以它必须向有知识的一方（AgentOS）查询。
        如果让有知识的一方直接做任务分解，这个查询就不需要了。

接口 2 (GuidanceSignal):
  AgentOS → 生成信号 → OpenClaw → 解析信号 → 调整调度
  问题: 为什么 AgentOS 不直接把指导信息嵌入它生成的计划中？
        因为 AgentOS 不生成计划——planning-with-files 生成。
        所以 AgentOS 只能"发送信号"给执行层。
        如果 AgentOS 自己生成计划，指导信息就是计划的一部分。

接口 3 (OutcomeFeedback):
  OpenClaw → 发送结果 → AgentOS → 调整置信度
  问题: 如果 AgentOS 自己在编排子任务，它天然就知道子任务的结果。
        不需要外部"发送"——结果在内部循环中直接可用。

接口 4 (MidSessionLearner):
  保留 — LLM 交互在 AgentOS 外部，监控它确实需要跨越边界。
```
...
| 陷阱预警 | AgentOS → GuidanceSignal | ProtoTask.common_pitfalls（从历史学习） |
| 阶段规划 | planning-with-files + LLM | 通用知识（每次从零开始） |
| 进度跟踪 | planning-with-files | 文件中的 checkbox 状态 |
| 结果学习 | AgentOS ← OutcomeFeedback | 置信度调整算法 |

有历史知识的一方（AgentOS）在做"辅助"——提供素材、发送信号、接收反馈。
...
| 职能 | V11 归属 | V12 归属 | 原因 |
|------|---------|---------|------|
| 任务类型识别 | AgentOS | AgentOS | 已有 TaskContext |
| 任务分解 | planning-with-files | **AgentOS (plan-generator)** | ProtoTask 是天然分解引擎 |
| 计划生成 | planning-with-files | **AgentOS (plan-generator)** | 从历史学习 > 从零生成 |
| 陷阱预警 | AgentOS (GuidanceSignal) | **AgentOS (嵌入计划)** | 计划本身包含陷阱信息 |
| 子任务编排 | OpenClaw | **AgentOS (task-orchestrator)** | 嵌套 while() 循环 |
| 验收检查 | 无系统化机制 | **AgentOS (verifier)** | 5 种验收标准 |
| 陷阱监控 | 无 | **AgentOS (pitfall-tracker)** | 主动匹配 + 反馈学习 |
| 进度跟踪 | planning-with-files | **AgentOS (progress-tracker)** | 状态机驱动 |
| 文件持久化 | planning-with-files | planning-with-files（降格） | 仍然需要文件 |
| 结果学习 | AgentOS (OutcomeFeedback) | **AgentOS (内部闭环)** | 不需要外部反馈 |
| LLM 交互监控 | AgentOS (MidSessionLearner) | AgentOS (MidSessionLearner) | 保留——LLM 在外部 |
...
V12: 29 个模块，1 个外部接口，2 个架构组件（AgentOS + OpenClaw）
...
**如果没有**：任务状态是扁平的——TaskContext 只知道"当前在做什么任务"，不知道任务的结构（几个阶段、哪些子任务、依赖关系）。任务跨 session 时，AgentOS 无法判断"上次做到哪了，接下来该做什么"。

**有了之后**：TaskOrchestrationState 维护完整的任务结构。session_start 时恢复状态，session_end 时验收并推进。跨 session 的任务连续性是确定的，不是靠 LLM "回忆"。

### 内层循环 — 必要性论证

**如果没有**：子任务执行是黑盒。AgentOS 注入了上下文后就"旁观"——不知道子任务在进行中还是已完成，不知道用户是否纠正了错误，不知道工具调用是否偏离了子任务范围。
...
**V12**：外层循环由 session_start / session_end Hook 驱动。用户每次开始新会话 → AgentOS 推进任务状态。这是"被动推进"——但状态机本身是完整的。

**V13**：外层循环可以由 `scheduleSessionTurn()` 主动触发。AgentOS 在 session_end 时计算"下一个子任务需要启动"，设置定时触发。不需要等用户手动开始下一个 session。

内层循环也可以升级——`subagent.run()` 让 AgentOS 对独立的子任务 spawn 子 Agent 并行执行，而不是在当前 session 中串行。

---

## 五、反向论证

**反方论点**："planning-with-files 经过了充分验证（benchmark 96.7% pass rate）。AgentOS 接管后引入的复杂度（6 个新模块、状态机、验收标准）可能降低可靠性。V11 的四个接口方案更保守、更安全——保持 planning-with-files 不变，只增强接口。"

**回应**：

1. planning-with-files 的 96.7% benchmark 是在**没有 AgentOS 知识加持**的情况下测的。AgentOS 接管后，计划生成质量应该更高（ProtoTask 提供基于历史的阶段划分和陷阱预警），不是更低。
...
- 如果 plan-generator 基于 bootstrap ProtoTask 生成的计划质量显著低于 planning-with-files + LLM 从零生成（在盲评 A/B 测试中，planning-with-files 胜出 > 60%）→ AgentOS 不应接管计划生成
