...
- step: "客户验收"
  ...
  praxis_role: "facilitator"         # Praxis 是组织者
...
ProcessTemplate 不是写死的——Praxis 从实际执行中学习并优化模板：

```
初始模板（运维者预置）:
  需求 → 设计 → 开发 → 测试 → 验收

第 5 次执行后（Praxis 从 ActionVerification 中学到）:
