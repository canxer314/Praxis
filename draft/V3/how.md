# How does Praxis V3 work?

## 总览：六层架构 + Curiosity Engine

Praxis V3 保持六层架构，但每一层都有扩展。新增的 **Curiosity Engine** 是一个横切关注点——嵌入在 L3（知识管理）和 L4（学习闭环）中，由 L6（自主决策）治理。
...
V3 的工具层与 V2 完全相同。工具由 OpenClaw 注册和执行，Praxis 管理元数据和熟练度追踪。
...
任务 "周报-膜力云-W3" → 工具链: [data_fetch, chart_gen, ppt_gen, email_send, team_msg]
                        ↑
Praxis: 检测到稳定的工具链模式 → 提取为 TaskPattern
```
...
KnowledgeGap:
  检测来源:
    - 用户在对话中反复提到一个概念，但 Praxis 知识库中无定义
    - 执行任务时遇到未知术语/系统/干系人
    - 任务结果被用户纠正，且纠正涉及 Praxis 不理解的业务逻辑
...
四维能力模型支持**雷达图**展示，用户通过 `/praxis status` 查看：
...
curiosity_decision:
  ...
  决策矩阵:
    ...
    Level 1 (标记缺口):
      → 检测 + 标记缺口，不提问
      → session_end 时展示给用户："/praxis gaps" 可查看
...
Level 1 → Level 2: 用户手动升级（/praxis curiosity level 2）
Level 2 → Level 3: 多维度 proficiency ≥ 0.7 且用户手动升级
```

**关键设计原则**：从 Level 1 到 Level 2 的升级必须是**用户手动**的——Praxis 不能单方面决定开始主动打扰用户。
