...
| **Hermes** | 已内置学习闭环（Honcho + skill creation），与 Praxis 学习系统形成"双主权"冲突 |
| **OpenClaw** | Memory 是插件槽（可替换）、核心不含学习系统、Hook 系统完整——架构干净 |

**关键洞察**：Hermes 的 README 自述为 "The self-improving AI agent — the only agent with a built-in learning loop"。Praxis 要做的事情正是"通用的学习闭环中间件"。Hermes 和 Praxis 解决的是同一层问题——但 Hermes 是作为 agent 的一部分，Praxis 是作为独立中间件。这使得它们在架构上更接近**替代关系**而非**互补关系**。

### 为什么 AgentMemory 做存储后端？

- AgentMemory 提供了 Praxis 约 75% 的存储需求：能力模型（slot）、知识条目（typed memory）、学习事件（lesson）、会话管理（sessions）、模式检测（patterns）
...
这意味着 Praxis 不需要"寄生"在 OpenClaw 的钩子上，而是**合法地占据一个架构槽位**。OpenClaw 的核心团队**预期并设计了**这种替换。

## 追问 5 层"为什么"

### 第 1 层：为什么需要身体层（body）而不是自建所有工具？

因为"工具的多样性 × 维护成本"远超一个团队能承担的范围。OpenClaw 已有的 20+ 消息通道、浏览器控制、终端执行、MCP 客户端——这些不是 Praxis 的核心竞争力，但它们是 Praxis 存在的前提。

### 第 2 层：为什么身体层必须没有自己的学习系统？

因为如果身体层有自己的学习系统，Praxis 的学习闭环就无从验证。AI 变好了——是 Praxis 的能力模型在起作用，还是身体层自己的 skill 在进化？不可归因 = 不可信任 = 不可优化。
...
### 第 4 层：为什么 Praxis 不自己实现 Hook 系统？

因为 Hook 的触发点在 LLM ↔ Tool 交互的每一帧——这意味着必须在 agent runtime 内部。自建 agent runtime 等于重新造 OpenClaw 的轮子。

### 第 5 层：为什么这个系统必须是统一的（Praxis + OpenClaw + AgentMemory 三者绑定）？

因为它们各自负责不可替代的一层：

```
AgentMemory   = 持久化的"大脑皮层"（存储、检索、固化）
Praxis       = 学习与决策的"心智模型"（能力评估、自主决策、经验提取）
OpenClaw      = 行动的"身体"（工具执行、通道通信、感知输入）
```

**三者缺一不可**。没有 AgentMemory → 经验丢失；没有 Praxis → 不会成长；没有 OpenClaw → 没有手脚。

## 一个不可否认的事实

> OpenClaw 给了 AI 手和脚，AgentMemory 给了 AI 笔记本，Praxis 给了 AI 从笔记和实践中变聪明的能力。
