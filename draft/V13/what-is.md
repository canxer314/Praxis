...
> V13 = 完整认知操作系统。V12 实现了记忆+学习+编排，V13 激活第四个核心职能——**驱动**。Praxis 不再只是"响应 hook"，而是主动发起会话、并行执行子任务、持续监控任务健康。

## 一句话定义

**Praxis V13 是一个完整的认知操作系统——具备记忆（结构化知识存储）、学习（置信度驱动的知识成长）、编排（任务状态机+验收）、驱动（主动调度+并行执行+健康监控）四个核心职能。它位于 LLM 和世界之间，把 LLM 每次会话的"空白大脑"状态，转换为"有记忆、有经验、有任务意识、能自主推进"的连续认知存在。**
...
V12:    从"被动记忆"到"主动认知引擎" — 任务编排状态机
        • 拆除 planning-with-files 边界，Praxis 直接做任务分解
...
| 主动调度下一个 session | Praxis 绕过用户控制（GovernancePolicy 控制） |
| 并行执行独立子任务 | 替代所有串行执行 |
...
| 维度 | V12 | V13 | 变化 |
|------|-----|-----|------|
| 架构哲学 | Praxis 直接做任务分解 | **Praxis 主动驱动任务执行** | 职能扩展 |
...
| 会话触发 | 用户手动 | **用户手动 + Praxis 自动** | +autonomy |
...
- [Why Praxis V13?](why.md) — 第一性原理：为什么被动响应不够，为什么需要主动驱动
