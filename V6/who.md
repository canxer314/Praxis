# Who is Praxis V6 for?

## 角色全景（V6：人类作为递归终止条件）

V5 的人类角色是"治理者"——审核新结构提案、设定边界、紧急刹车。但 V5 中 Meta Layer 不能修改自己，所以治理者不需要处理"谁来审核 Meta Layer 的修改"这个问题。

V6 打破了这个限制。当 Praxis 可以修改任何认知结构——包括 Meta Layer 本身、包括层定义、包括"层"这个概念——出现了一个递归问题。
...
```
V1-V5 的交互模式:
  用户给指令 → Praxis 执行 → 用户反馈

V6 新增的交互模式:
  用户带 Praxis 进入陌生场景 →
  Praxis 观察 → Praxis 提问（感知性问题）→
  用户回答 → Praxis 形成假设 →
  用户验证/纠正 → Praxis 迭代 →
  模式涌现 → Praxis 提出固化 →
  场景不再陌生
...
```
Praxis: "我注意到这里的人都穿着白色衣服。这是这里的人特定的着装，还是只是巧合？"

用户: "白大褂是医护人员的标志。白色代表卫生。"

Praxis: "明白了。所以穿白衣服的人属于'医护人员'这个类别。
...
Praxis: "好的。ProtoRole['医生']: 诊断者。ProtoRole['护士']: 执行者。
         置信度 0.6。我会继续观察来确认这个区分。"

# 这种对话不是"用户教 Praxis 具体知识"
# 而是"Praxis 主动提问来帮助自己构建对陌生场景的理解框架"
```

### 用户的新角色：感知教练

V1-V4 中用户教 Praxis 的是**知识**（"膜力云有三个数据源"）和**流程**（"先确认数据新鲜度再做报告"）。
...
|------|---------|---------|
| **人类是什么角色？** | 治理者 | **递归终止条件** |
| **Praxis 怎么应对零先验场景？** | 不能（需要模板匹配作为锚点） | **协同感知 → 原型构造 → 互动精化** |
