...
│                                                           │
│  紧急刹车:                                                │
│  • 用户随时可以: /agentos architecture freeze            │
...
| 命令 | 功能 |
|------|------|
| `/agentos perceive` | 查看当前零先验场景的感知状态（SalientElement + ProtoStructure） |
| `/agentos proto <id>` | 查看某个原型的详细信息 + 置信度历史 |
| `/agentos proto <id> correct` | 纠正某个原型（加速精化） |
| `/agentos architecture` | 查看当前架构版本 + 层定义 |
| `/agentos architecture proposals` | 查看待审批的架构变更提案 |
| `/agentos architecture freeze` | 紧急冻结所有 Level 3+ 修改 |
| `/agentos architecture unfreeze` | 解冻 |
| `/agentos architecture rollback <version>` | 回滚架构到指定版本 |
