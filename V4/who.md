# Who is AgentOS V4 for?

## 角色全景（V4 核心变化：从一个人到一张关系网）

V3 的角色模型中心是**用户**——AgentOS 理解用户、向用户学习、在合适时机向用户提问。用户是唯一被建模的人。

V4 承认一个现实：**大多数任务不止一个人**。AgentOS 需要理解和协调的不只是用户，而是一整张关系网。
...
**V4 的体验**：
> 用户："把用户管理模块做了"
> AgentOS："收到。我会按标准流程推进——先找 PM 张三确认 PRD，再找架构师李四沟通方案，然后协调开发经理王五排期，开发完成后找测试赵六验证，最后请 PM 和客户验收。有问题随时跟我同步。"
>
> （AgentOS 自己推动流程，卡住时才找用户，不卡就默默推进）
...
每个协作者通过自己的渠道（飞书、邮件、OpenClaw 通道）与 AgentOS 交互：
...
> 张三回复："周四下午可以，发我会议室邀请。"
>
> AgentOS：更新 ProcessInstance → 创建会议 → 提醒用户

> **架构师李四** 收到邮件：
> "李四，用户管理模块的 PRD 已确认（附后）。有三个技术选型需要你的意见：认证方案选 JWT 还是 Session？权限模型用 RBAC 够吗？用户数据分表策略怎么定？方便的时候回复即可，不急。"
>
> 李四 3 天后回复 → AgentOS 检测到步骤出口条件满足 → 进入下一阶段

### 协作者不需要知道 AgentOS 的存在

对协作者来说，他们收到的消息来自"膜力云项目的 AI 助手"——他们不需要理解 AgentOS 是什么。他们只需要回复，AgentOS 负责：
- 理解回复内容
- 更新流程状态
- 触发下一步
- 在需要时升级给用户
...
| 隐私与边界 | AgentOS 能看到用户与协作者的通信内容吗？权限边界在哪里？ |
| ProcessTemplate 维护 | 谁来定义和维护流程模板？V4 默认：运维者预置通用模板 + AgentOS 从实践中学习优化 |
...
| **AgentOS 在培养谁？** | 一个会学习、会提问的 AI | 一个**能推动流程、协调角色**的 AI |
