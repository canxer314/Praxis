# Who is Praxis V3 for?

## 角色全景（V3 更新：角色反转）

V3 最显著的角色变化：**Praxis 不再是纯粹的被教导者——它也会主动向用户提问。**

这与 V1/V2 形成了根本性的不对称：

```
V1/V2:  用户 ──教导──▶ Praxis ──执行──▶ 工具
                       （单向）

V3:     用户 ◀──提问── Praxis ──执行──▶ 工具
           ──教导──▶           （双向）
```
...
| 用户查看 AI 状态 | `/praxis status` → 工具熟练度 | `/praxis status` → 四维雷达 + 待确认的知识缺口列表 |

**导师的新体验**：

> 周五下午 3 点，Telegram：
>
> **Praxis**："这周学到了不少。顺便问一下——有个概念我一直不太确定。你在周报里提到的'数据对接'具体是指系统之间的 API 对接，还是也包括人工报表的整合？我注意到你在不同场合的用法好像不太一样。"
>
> **用户**："好问题。确实不一样。API 对接是说系统自动传数据，人工整合是说从 Excel 导入。两个都在做，但优先级不同……"
>
> **Praxis**："明白了，我记下了。下次周报我会区分这两个概念。"
...
| 审核机制 | 首次主动提问需要用户确认开启此模式；用户随时可以 `/praxis curiosity off` |
...
面向 MCP/OpenClaw Plugin SDK 开发工具。可选提供 `praxis` 元数据加速学习。无变化。
...
与 V2 相同的部署模式，但每个 Praxis 实例的能力模型现在包含四个维度：
...
OpenClaw Gateway
  ...
  └─ Agent "coding"
      └─ Praxis Plugin (scope="coding")
          ...
          ├─ domains: {"Praxis架构": 0.80, "TypeScript": 0.60}
          └─ ...
```
